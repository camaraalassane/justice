<template>
    <AuthenticatedLayout title="Tableau de Bord" subtitle="Statistiques et analyses">
        <div class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <Card>
                    <div class="flex items-center justify-between mb-3">
                        <h3 class="text-sm font-semibold text-gpj-700">👥 Répartition par type de personnel</h3>
                    </div>
                    <div class="grid grid-cols-2 gap-4">
                        <div v-for="item in statsParTypePersonnel" :key="item.type_personnel" 
                            ...
                            <p class="text-xs text-gpj-400">{{ item.nombre_procedures }} procédure(s)</p>
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Card>
                    <h3 class="text-sm font-semibold text-gpj-700 mb-3">⚖️ Infractions par type de personnel</h3>
                    <div class="space-y-2">
                        <div v-for="item in statsInfractionsParType" :key="item.type_personnel" 
                            ...
                            <span class="text-lg font-bold" :class="item.type_personnel === 'militaire' ? 'text-gpj-600' : 'text-purple-600'">
                                {{ item.nombre }}
                            </span>
                        </div>
                    </div>
                ...
                <Card>
                    <h3 class="text-sm font-semibold text-gpj-700 mb-3">📋 Fautes par type de personnel</h3>
                    <div class="space-y-2">
            ...
            <Card title="📊 Détail par lieu de commission et phase">
                ...
                <p v-else class="text-sm text-gpj-400 py-4 text-center">Aucune donnée disponible</p>
            </Card>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <Card class="md:col-span-2">
                    <h3 class="text-sm font-semibold text-gpj-700 mb-3">🏛️ Procédures par type de parquet</h3>
                    <div class="grid grid-cols-2 gap-3">
                        <div v-for="item in statsParquets" :key="item.parquet_type" class="p-3 rounded-lg text-center" 
                            ...
                            <p class="text-xs text-gpj-400">procédure(s)</p>
                        </div>
                    </div>
                </Card>
            </div>
            ...
            <Card title="📊 Détail des parquets">
                <div class="mb-3 flex flex-wrap gap-2">
                    <span class="text-xs text-gpj-400">Chaque parquet avec son nombre de procédures associées</span>
                </div>
                        ...
                        <tbody class="divide-y divide-gpj-100">
                            <tr v-for="(item, index) in statsParquetDetail" :key="item.id" class="hover:bg-slate-50 transition-colors">
                                ...
                                <td class="px-3 py-2 font-medium text-gpj-800">
                                    {{ item.nom }}
                                    <span v-if="item.nombre_procedures === 0" class="text-[10px] text-gpj-400 ml-1">(inactif)</span>
                                </td>
                                <td class="px-3 py-2">
                                ...
                                <td class="px-3 py-2 text-gpj-600">{{ item.localisation || '-' }}</td>
                ...
                <p v-else class="text-sm text-gpj-400 py-4 text-center">Aucun parquet enregistré</p>
            </Card>
                    ...
                    <div v-if="topFautes.length" class="space-y-2">
                        <div v-for="(faute, i) in topFautes" :key="i" class="flex items-center gap-2 text-sm">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <span v-if="faute.categorie" class="text-[10px] text-gpj-400">Catégorie: {{ faute.categorie }}</span>
                            </div>
            ...
            <Card title="📂 Fautes par catégorie">
                ...
                <p v-else class="text-sm text-gpj-400 py-4 text-center">Aucune faute enregistrée</p>
            </Card>
