<template>
    <GuestLayout title="Connexion" subtitle="Accédez à votre espace GPJ">
        <form @submit.prevent="submit" class="space-y-5">
            ...
            <!-- Mot de passe -->
            <div>
                <label for="password" class="block text-sm font-medium text-slate-800 mb-1">Mot de passe</label>
                <div class="relative">
                    ...
                    <input
                        ...
                        class="w-full pl-10 pr-10 py-2.5 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500 focus:border-slate-500"
                    />
                    <button type="button" class="absolute right-3 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-700" @click="showPassword = !showPassword">
            ...
            <div class="flex items-center justify-between">
                <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer">
                    <input type="checkbox" v-model="form.remember" class="rounded border-slate-400 text-slate-600 focus:ring-slate-500" />
                    Se souvenir de moi
                </label>
                <Link :href="route('password.request')" class="text-sm text-slate-600 hover:text-slate-800 font-medium">
                    Mot de passe oublié ?
                </Link>
            </div>
            ...
            <p class="text-center text-sm text-slate-500">
                ...
                <Link :href="route('register')" class="text-slate-600 hover:text-slate-800 font-medium">Créer un compte</Link>
            </p>
        </form>
    </GuestLayout>
</template>
