        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <Card>
                <div class="text-center mb-4">
                    <div class="w-20 h-20 rounded-full bg-slate-100 flex items-center justify-center text-slate-700 font-bold text-2xl mx-auto mb-3">
                        {{ personnel.nom ? personnel.nom.charAt(0) : '?' }}{{ personnel.prenoms ? personnel.prenoms.charAt(0) : '' }}
                    </div>
                    <h3 class="text-lg font-bold text-slate-900">{{ personnel.nom || 'Nom inconnu' }} {{ personnel.prenoms || '' }}</h3>
                ...
                <div class="space-y-3 text-sm">
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        ...
                        <span class="font-medium text-slate-900">{{ personnel.type_personnel === 'militaire' ? 'Militaire' : 'Civil' }}</span>
                    </div>
                    <div v-if="personnel.type_personnel === 'militaire'" class="flex justify-between py-2 border-b border-slate-200">
                        ...
                        <span class="font-medium text-slate-900">{{ personnel.matricule || 'Non défini' }}</span>
                    </div>
                    <div v-if="personnel.type_personnel === 'militaire'" class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Grade</span>
                        <span class="font-medium text-slate-900">{{ personnel.grade?.libelle || personnel.grade || '-' }}</span>
                    </div>
                    <div v-if="personnel.type_personnel === 'militaire'" class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Catégorie</span>
                        <span class="font-medium text-slate-900">{{ personnel.grade?.categorie?.libelle || '-' }}</span>
                    </div>
                    <div v-if="personnel.type_personnel === 'civil'" class="flex justify-between py-2 border-b border-slate-200">
                        ...
                        <span class="font-medium text-slate-900">{{ personnel.profession || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Genre</span>
                        <span class="font-medium text-slate-900">{{ personnel.genre || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Date de naissance</span>
                        <span class="font-medium text-slate-900">{{ formatDate(personnel.date_naissance) || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Lieu de naissance</span>
                        <span class="font-medium text-slate-900">{{ personnel.lieu_naissance || '-' }}</span>
                    </div>
                    ...
                    <div v-if="personnel.type_personnel === 'militaire'" class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Armée/Service</span>
                        <span class="font-medium text-slate-900">{{ armeeNom || '-' }}</span>
                    </div>
                    <div v-if="personnel.type_personnel === 'militaire'" class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Unité</span>
                        <span class="font-medium text-slate-900">{{ personnel.unite || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Adresse</span>
                        <span class="font-medium text-slate-900">{{ personnel.adresse || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                        <span class="text-slate-500">Téléphone</span>
                        <span class="font-medium text-slate-900">{{ personnel.telephone || '-' }}</span>
                    </div>
                    <div class="flex justify-between py-2 border-b border-slate-200">
                ...
                <div class="mt-4 space-y-2">
                    ...
                    <Link
                        ...
                        class="w-full flex items-center justify-center gap-2 px-3 py-2.5 border border-slate-300 text-slate-700 text-sm font-medium rounded-lg hover:bg-slate-50 transition-colors"
                    >
                    ...
                    <div v-if="personnel.procedures?.length" class="space-y-3">
                        ...
                        >
                            <div class="w-10 h-10 rounded-lg bg-slate-100 flex items-center justify-center shrink-0">
                                <i class="pi pi-folder text-slate-600"></i>
                            </div>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-slate-900">{{ procedure.numero_procedure }}</p>
                                <p class="text-xs text-slate-500">{{ formatDate(procedure.date_ouverture) || formatDate(procedure.created_at) || 'Date inconnue' }}</p>
                                <p v-if="procedure.parquet" class="text-xs text-slate-500">
