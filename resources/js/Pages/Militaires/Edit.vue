    ...
    <AuthenticatedLayout title="Modifier Personnel" :subtitle="`${personnel.matricule || 'Sans matricule'} - ${personnel.nom} ${personnel.prenoms}`">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
                        <div v-if="form.type_personnel === 'militaire'">
                            ...
                            <input v-model="form.matricule" type="text" placeholder="MIL-2026-XXX"
                                class="w-full px-3 py-2.5 rounded-lg border border-slate-300 text-sm uppercase focus:outline-none focus:ring-2 focus:ring-slate-500" />
                            <p class="text-xs text-slate-500 mt-1">Laissez vide pour conserver l'actuel</p>
                        ...
                        <div v-if="form.type_personnel === 'militaire'">
                            ...
                            <div v-if="form.armee_id === '__nouveau__'" class="mt-2 flex gap-2">
                                <input 
                                    ...
                                    class="flex-1 px-3 py-2 rounded-lg border border-slate-300 text-sm focus:outline-none focus:ring-2 focus:ring-slate-500"
                                />
                                <button 
                                    type="button"
        ...
        <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="bg-white dark:bg-slate-900 rounded-xl p-6 max-w-md w-full mx-4 shadow-xl">
                ...
                <p class="text-sm text-slate-700 mb-2">Vous êtes sur le point de supprimer :</p>
                <p class="text-sm font-bold text-slate-900 mb-2">{{ personnel.matricule }} - {{ personnel.nom }} {{ personnel.prenoms }}</p>
                ...
                <div class="flex gap-3 justify-end">
                    <button @click="showDeleteModal = false" class="px-4 py-2 border border-slate-300 text-slate-700 text-sm rounded-lg hover:bg-slate-50 cursor-pointer">Annuler</button>
