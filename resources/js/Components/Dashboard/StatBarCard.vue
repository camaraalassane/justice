<template>
    <Card>
        <div class="flex items-center justify-between mb-3">
            ...
            <a v-if="exportUrl" :href="exportUrl" class="text-xs text-slate-600 hover:text-slate-800 flex items-center gap-1 shrink-0">
        ...
        <div v-if="data.length" class="space-y-2">
            <div v-for="(item, i) in data" :key="i" class="flex items-center gap-2 text-xs">
                <span class="w-5 h-5 rounded bg-slate-100 text-slate-700 flex items-center justify-center text-xs font-bold shrink-0">{{ i + 1 }}</span>
                ...
                <span class="font-bold text-slate-700 w-8 text-right">{{ item.nombre }}</span>
            </div>
        </div>
